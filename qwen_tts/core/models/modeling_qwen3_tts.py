        ...
        # Generate
            ...
            if use_crossbar and actual_steps > 0:
                ...
                if actual_steps < total_groups:
                    ...
                    batch_size = input_ids.shape[0]
                    pad_count = total_groups - actual_steps
                    
                    zero_seqs = torch.zeros((batch_size, pad_count), dtype=dtype, device=device)
                    codec_ids = torch.cat((input_ids, pred_seqs, zero_seqs), dim=-1)
                    
                    pad_embeds = [embed_layers[i + actual_steps](torch.zeros((batch_size, 1), dtype=dtype, device=device))
                ...
                dtype = input_ids.dtype
                batch_size = input_ids.shape[0]
                zero_seqs = torch.zeros((batch_size, total_groups), dtype=dtype, device=device)
